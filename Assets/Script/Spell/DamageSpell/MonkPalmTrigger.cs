    ...
    public class MonkPalmTrigger : DamageSpellTrigger
    {
        ...
        protected override void EntityHit(BoardEntity entity, TriggerSpellData spellData, Vector2Int origin, CastInfo castInfo)
        {
            ...
            for (int i = 0; i < m_KnockDistance; i++)
            {
                ...
                {
                    entity.Buffs.AddBuff(new StunDebuff(spellData.AttachedEntity,entity,BuffType.StunDebuff,BuffGroup.Debuff,m_StunDuration,2,BuffConst.Stun,false),VisualEffectType.StunStars);
                    ...
                    if (collideEntity)
                    {
                        collideEntity.Buffs.AddBuff(new StunDebuff(spellData.AttachedEntity,collideEntity,BuffType.StunDebuff,BuffGroup.Debuff,m_StunDuration,2,BuffConst.Stun,false),VisualEffectType.StunStars);
